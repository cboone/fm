...
## Unsubscribe command help

```scrut
$ $TESTDIR/../fm unsubscribe --help
Extract and display the List-Unsubscribe header from an email. (glob)
* (glob+)
Usage: (glob)
  fm unsubscribe [email-id] [flags] (glob)
 (regex)
Flags: (glob)
*--after* (glob)
*--before* (glob)
