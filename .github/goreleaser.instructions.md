...
- **Shell completions are pre-generated**: The `before.hooks` step runs `scripts/completions.sh` which builds the binary and generates bash, zsh, and fish completion scripts into `completions/`. These are included in archives and referenced in `homebrew_casks.completions`. Do not suggest removing the before hook or the completions field.
